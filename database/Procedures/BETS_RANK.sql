CREATE DEFINER=`panda_dev`@`%` PROCEDURE `BETS_RANK`(
...
proc_label:BEGIN
	...
	SET @BACCARAT = 'banker,player,tie,banker_pair,player_pair,super_six';
	SET @DT = 'dragon,tiger,dt-tie';
	SET @MONEYWHEEL = '1,2,5,10,20,og';
	SET @ROULETTE = 5;
	SET @THREECARDS = '3c-phoenix,3c-dragon,3c-lucky';
	SET @NIUNIU = 7;
	...
	WHERE
		gc.id = GAME_CODE_ID
		AND (
			FIND_IN_SET(bet_place,@BACCARAT)
			OR FIND_IN_SET(bet_place,@DT)
			OR FIND_IN_SET(bet_place,@MONEYWHEEL)
			OR bp.gamecode_id = @ROULETTE
			OR FIND_IN_SET(bet_place,@THREECARDS)
			OR bp.gamecode_id = @NIUNIU
		)
	GROUP BY
		bp.bet_place;
